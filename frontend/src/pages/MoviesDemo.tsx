...
// import { createMovieMultipart, updateMovieSmart, listGenres, Movie, Genre } from "../../features/movies"
...
//   const [ageRating, setAgeRating] = useState(movie?.age_rating ?? '')
//   const [linkYtb, setLinkYtb] = useState(movie?.link_ytb ?? '')
//   const [type, setType] = useState<'trailer'|'full'>((movie as any)?.type ?? 'trailer')
...
//   // Load genres (name dropdown)
//   useEffect(() => {
//     (async () => {
//       try {
//         const gs = await listGenres()
//         setGenres(gs)
//       } catch (e) {
//         console.error('Không tải được genres', e)
//       }
//     })()
//   }, [])

//   function normalizeQuality(v: string) { return v ?? '' }
...
//     try {
//       if (isEdit) {
//         // Update thông minh: nếu có file → multipart; không file → JSON
...
//         })
//       } else {
//         // CREATE — multipart bắt buộc có poster; nếu full thì thêm movie_url
//         if (!posterFile) {
//           setMsg('Vui lòng chọn poster (bắt buộc).')
//           setSubmitting(false)
//           return
//         }
...
//             <select className="rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-2"
//                     value={type} onChange={e=>setType(e.target.value as 'trailer'|'full')}>
...
//           {/* Poster (create bắt buộc / edit tuỳ chọn) */}
//           <label className="flex flex-col gap-1">
